...
private:
    ...
    int m_otpLength;
    int m_otpExpiryTime; // thời gian hết hạn tính bằng giây
...
public:
    ...
    std::string generateOTP()
    {
        ...
        // Sử dụng random engine an toàn hơn
    ...
    // Callback function cho CURL để xử lý phản hồi
    ...
    bool sendOTP(const std::string &recipientEmail, std::string &otp)
    {
        ...
        std::string emailBody = "Subject: Your OTP Code\r\n"
                                ...
                                std::to_string(m_otpExpiryTime / 60) + " phút.\r\n";
        ...
        if (!success)
        {
            std::cerr << "Lỗi khi gửi email: " << curl_easy_strerror(res) << std::endl;
        }
